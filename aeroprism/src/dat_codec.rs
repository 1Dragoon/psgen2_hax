...
#[inline]
pub async fn parse_dat_header<T: AsyncBufReadExt + Unpin>(dat_reader: &mut T) -> Vec<usize> {
    // Header:
    ...
    header_reader.read_exact(&mut data_field).await.unwrap();
...
#[inline]
pub async fn pack_dat_components(path: &PathBuf) -> BTreeMap<u16, Vec<u8>> {
...
#[inline]
pub async fn pack_dat(path: &PathBuf, dest: &PathBuf) {
